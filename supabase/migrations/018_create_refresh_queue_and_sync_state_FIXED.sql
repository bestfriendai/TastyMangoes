...
--  Notes: Create refresh_queue and sync_state tables for daily refresh of existing movies
--  UPDATED: Ensures works table exists before creating refresh_queue

-- ============================================================================
-- STEP 1: Ensure works table exists (create if missing)
-- This ensures compatibility whether tasty_mangoes_schema.sql was run or not
-- ============================================================================
DO $$
BEGIN
  -- Only create works table if it doesn't exist
  IF to_regclass('public.works') IS NULL THEN
    CREATE TABLE public.works (
        ...
        -- Ingestion status (add if not exists)
        ...
        -- Constraint for ingestion_status
        CONSTRAINT works_ingestion_status_check
    ...
    -- Basic RLS policies (service_role can do everything)
    ...
    RAISE NOTICE 'Created works table (was missing)';
  ELSE
    -- Works exists, but ensure ingestion_status column exists
    IF NOT EXISTS (
        ...
        RAISE NOTICE 'Added ingestion_status column to existing works table';
    END IF;
    
    -- Ensure last_refreshed_at exists
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
        WHERE table_schema = 'public' 
        AND table_name = 'works' 
        ...
        RAISE NOTICE 'Added last_refreshed_at column to existing works table';
    END IF;
  END IF;
END $$;
